...
public class ExportCollectionController implements Initializable {
    ...
    private void handleExportCollectionButton(ActionEvent event) {
        if (!FileUtils.isDirectory(selectedDirectory)) {
            triggerErrorAlert("Invalid directory path selection", "Select a valid directory path to continue");
            return;
        }
        ...
        if (CollectionUtils.isEmpty(collections)) {
            triggerErrorAlert("Collection Not Found!", "Add Collection before export");
        } else {
